...
public class bl_PhotonConnection : Photon.MonoBehaviour {
	...
	void Awake (){
        ...
        if (m_SaveInfo != null)
        {
            ...
            if (m_SaveInfo.m_UserName != string.Empty && m_SaveInfo.m_UserName != null)
            {
                ...
                m_Kills = m_SaveInfo.m_Kills;
                m_Deaths = m_SaveInfo.m_Deaths;
                m_Score = m_SaveInfo.Score;
    ...
    void ShowPlayerInfo()
    {
        GUILayout.BeginArea(new Rect(0, Screen.height / 2, 250, 200));
        GUILayout.Label("Name: <color=orange>" + playerName + "</color>");
        GUILayout.Label("Kills: <color=orange>" + m_Kills + "</color>");
        GUILayout.Label("Deaths: <color=orange>" + m_Deaths + "</color>");
        GUILayout.Label("Score: <color=orange>" + m_Score + "</color>");
        GUILayout.EndArea();
    }
	...
	void MainMenu (){
        ...
        if (GUILayout.Button("Customizer", GUILayout.Height(40)))
        {
            PlayAudioClip(a_Click, transform.position, 1.0f);
            Application.LoadLevel("Customizer");
        }
        if (GUILayout.Button("Ranking", GUILayout.Height(40)))
        {
            PlayAudioClip(a_Click, transform.position, 1.0f);
            Application.LoadLevel("Ranking");
        }
        if (GUILayout.Button("Log Out", GUILayout.Height(GetButtonSize(LobbyState.Quit))))
        {
            m_state = LobbyState.Quit;
            PlayAudioClip(a_Click, transform.position, 1.0f);
        }
	...
	void CreateServer (){
		...
		//Create a new Room with Photon		
				if(GUI.Button( new Rect(150,330,175,50),"Create Game")) { 
                    ...
                    PhotonNetwork.CreateRoom(hostName, new RoomOptions() { maxPlayers = (byte)maxPlayers[players], isVisible = true,isOpen = true,
	...
	void QuitGame (){
        ...
        GUILayout.Space(35);
        GUILayout.Label("Are you sure you want to Log Out?");
        GUILayout.Space(10);
        GUILayout.BeginHorizontal();
        GUILayout.Space(35);
