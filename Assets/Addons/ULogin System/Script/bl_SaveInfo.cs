...
public class bl_SaveInfo : MonoBehaviour {
    ...
    /// get curent player propiertis info for send and convert in int
    /// </summary>
    ...
    {
        int kp = PhotonNetwork.player.GetKills();
        int dp = PhotonNetwork.player.GetDeaths();
        int sp = PhotonNetwork.player.GetPlayerScore();
        SaveInfo(kp, dp, sp);
    }
    /// <summary>
    /// If we connect then is avaible for update
    ...
    {
        if (SaveInfoPHP_Url != string.Empty)
        {
    ...
    IEnumerator Save(int k,int d, int s)
    {
        ...
        if (www.error == null)
        {
            ...
            {
                GameObject.Find(bl_LevelUnlock.LUName).GetComponent<bl_LevelUnlock>().UpdateScore(t_score);
            }
            //Update Vals Locals
            m_Kills = t_kills;
            m_Deaths = t_deaths;
