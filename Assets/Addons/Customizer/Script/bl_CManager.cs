...
public class bl_CManager : MonoBehaviour {
    ...
    {
        GUI.skin = Skin;
        GUILayout.BeginArea(new Rect(0, Screen.height - 45, Screen.width, 75));
        GUILayout.BeginHorizontal();
        for (int i = 0; i < AllCustom.Count; i++)
        {
            float h = ActualSelect(AllCustom[i]) == true ? 45 : 35;
            if (GUILayout.Button(AllCustom[i].gameObject.name,GUILayout.Height(h)))
            {
