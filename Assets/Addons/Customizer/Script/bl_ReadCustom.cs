...
public class bl_ReadCustom : MonoBehaviour {
    ...
    void OnEnable()
    {
        view = this.transform.root.GetComponent<PhotonView>();
        Read();
    }
    /// <summary>
    /// 
    ...
    void Read()
    {
        ...
        if (PlayerPrefs.HasKey(Go_Name))
        {
            ...
            if (view != null)
            {
                view.RPC("SyncCustomizer", PhotonTargets.OthersBuffered, m_Read, PhotonNetwork.player.name);
            }
            else
            {
                Debug.LogError("Cant get PhotonView");
            }
        }
    }
    /// <summary>
